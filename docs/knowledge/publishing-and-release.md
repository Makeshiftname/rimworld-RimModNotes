...
`78-RimFlixAnimeShows/AGENTS.md` 的做法：把 mod 目录软链接到游戏目录，改动即生效。
...
（见 [`78-RimFlixAnimeShows/AGENTS.md`](../../78-RimFlixAnimeShows/AGENTS.md)）。
...
[`16-ResearchPrerequisites/_PublisherPlus.xml`](../../16-ResearchPrerequisites/_PublisherPlus.xml)：
...
`78-RimFlixAnimeShows/AGENTS.md` 明确：**本地自用随意；AI 生成的碧蓝航线等角色图/视频
不要上传创意工坊**（角色 IP 属原厂）。发布前检查素材版权。

## 4. 补丁类 mod 的 About.xml 同步规则

`55-CommonModCompatibilityPatches/AGENTS.md` 约定（发布前必查）：
...
- 部署与版权：`78-RimFlixAnimeShows/AGENTS.md`
- 发布配置：`16-ResearchPrerequisites/_PublisherPlus.xml`
- 补丁发布约定：`55-CommonModCompatibilityPatches/AGENTS.md`

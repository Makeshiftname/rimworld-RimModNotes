# -*- coding: utf-8 -*-
"""RimWorld 简体中文翻译错误来源分析（最终版）。
输出：
  1. 全局缺失 Keyed / DefInjected 翻译数（英文有、中文无）—— 3887 的大头
...
MODS_ROOT = r"F:\Games\GBL\rimworld\Mods"
DATA_ROOT = r"F:\Games\GBL\rimworld\Data"
CONFIG_XML = r"F:\Games\GBL\Config\ModsConfig.xml"
...
with io.open(os.path.join(os.path.dirname(__file__), "tmp_translation_errors.txt"), "w", encoding="utf-8") as f:
    f.write("\n".join(out_lines))
print(f"\n[已导出] 完整确定错误清单 -> tools/tmp_translation_errors.txt ({len(out_lines)} 行)")
